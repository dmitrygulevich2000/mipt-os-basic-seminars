...
1. Остановить контейнер можно командой `docker stop os-env`, а затем удалить - `docker rm os-env`.
   При удалении внесённые в контейнер изменения удалятся (кроме тех, что были на примонтированных вольюмах)

1. Остановленный контейнер (например, после перезагрузки системы) можно запустить командой `docker start os-env`

С подобным окружением хорошо сочетаются средства IDE для удаленной разработки: [VSCode](https://code.visualstudio.com/docs/remote/ssh) (тестировал), [CLion](https://www.jetbrains.com/help/clion/clion-toolchains-in-docker.html) (не тестировал)
