...
ssize_t client_read(socket_state_t* client) {
    ...
    ssize_t rbytes =
        read(client->socket, client->buf, sizeof(client->buf));
    if (rbytes <= 0) {
        return rbytes;
    }
...
ssize_t client_write(socket_state_t* client) {
    ssize_t wbytes =
        write(client->socket, client->buf + client->buf_begin,
...
int handle_client_until_eagain(
    int epollfd, struct epoll_event* trigger_event) {
...
void handle_client_event(int epollfd,
    ...
    if (status < 0) {
        fprintf(
            stderr,
            "communication failed with error %d, cause: %s\n",
...
void accept_connections_until_eagain(
    int epollfd, const struct epoll_event* trigger_event) {
    int server_socket =
        ((socket_state_t*)trigger_event->data.ptr)->socket;
    ...
    while (1) {
        ...
        client_event.events =
            EPOLLHUP | EPOLLIN | EPOLLOUT | EPOLLET;
        ...
        // try handle client
        int status =
            handle_client_until_eagain(epollfd, &client_event);
        if (status < 0) {
            fprintf(stderr,
                    "communication failed with error %d, cause: "
                    "%s\n",
                    -status, strerror(errno));
            close_connection(epollfd, client);
        }
...
void handle_conn_event(int epollfd,
...
void run(int epollfd, int server_socket) {
    ...
    if (epoll_ctl(epollfd, EPOLL_CTL_ADD, server_socket,
                  &conn_event) < 0) {
    ...
    while (1) {
        int event_count =
            epoll_wait(epollfd, events, MAX_EPOLL_EVENTS, -1);
        if (event_count < 0) {
            err(1, "epoll_wait");
        }
        ...
        for (int i = 0; i < event_count; ++i) {
            ...
            if (((socket_state_t*)event->data.ptr)->socket ==
                server_socket) {
