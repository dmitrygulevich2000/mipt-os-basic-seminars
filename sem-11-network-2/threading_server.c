...
int handle_client(int client_socket) {
    ...
    while ((rbytes = read(client_socket, buffer, sizeof(buffer))) > 0) {
        ...
        while (rbytes > 0) {
            ssize_t wbytes = write(client_socket, write_data, rbytes);
            if (wbytes < 0) {
                return -2;
            }
            rbytes -= wbytes;
...
void* handler_thread(void* raw_socket) {
    int socket = *(int*)raw_socket;
    int status = handle_client(socket);
    if (status < 0) {
...
int launch_handler_thread(int client_socket) {
    int* argument = malloc(sizeof(int));
    *argument = client_socket;
    pthread_t thread;
    int status = pthread_create(&thread, NULL, handler_thread, argument);
    if (status != 0) {
        return -status;
    }
...
void run(int server_socket) {
    ...
    fprintf(stderr, "Connection received on %s\n", straddr(&client_addr));
        ...
        int status = launch_handler_thread(client_socket);
        if (status < 0) {
            fprintf(stderr, "failed to create handler thread: %s", strerror(-status));
        }
    }
}
